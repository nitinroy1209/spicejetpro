	...
	public void launchBrowser(String url)
	{
		try
		{
			...
			//options.setHeadless(true);
			 options.addArguments("--disable-notifications");	        // options.addArguments("incognito"); // You can uncomment this line if you want to enable incognito mode
		...
		//driver= new EdgeDriver();
			...
			driver.get(url);
			Reports.reportStep("PASS", "The chrome browser launched successfully with the given url ("+url+")");// generate log for each test case and if it is in try block it is pass
...
//send key	
	public void typeText(WebElement ele, String text)
	{
		try
		{
			waitForMe(ele,20);//WebElement is visible for maximum 20sec and for time interval of 200ms
			ele.clear();
			ele.sendKeys(text);
